<header class="w-full bg-gray-100" x-data="{open: false}">
    <div class="max-w-7xl px-3 flex items-center justify-between mx-auto py-3">
        <div class="flex items-center gap-4">
            ...
            <p class="text-xl font-semibold text-honey">Honig4U</p>
        </div>
        ...
        <div class="hidden sm:flex items-center gap-4">
            <?php if (isset($_SESSION['user'])) : ?>
                <div class="relative flex gap-3 items-center" x-data="{open: false}" @click="basketOpen = !basketOpen">
                    <p>Warenkorb</p>
                    <img src="images/basket.svg" class="w-10 h-10" alt="">
                </div>
            <?php else : ?>
                <a href="login.php" class="text-honey flex items-center gap-2 px-2 py-1 ring-2 ring-honeygreen rounded-md font-semibold">
                    Login
                </a>
                <a href="registrierung.php" class="flex items-center gap-2  px-2 py-1 ring-2 ring-honey rounded-md bg-honeygreen text-white font-semibold">
                    Registrieren
                </a>
            <?php endif; ?>
        </div>
    ...
    <div class="flex flex-col gap-2 border-t-2 px-2 sm:hidden" x-show="open" style="display:none">
        ...
        <div class="items-center gap-4">
            <?php if (isset($_SESSION['user'])) : ?>
                <div class="relative flex gap-3 items-center" x-data="{open: false}" @click="open = !open">
                    <p>Warenkorb</p>
                    <img src="images/basket.svg" class="w-10 h-10" alt="">
                </div>
            <?php else : ?>
